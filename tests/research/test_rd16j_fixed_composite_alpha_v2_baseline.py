...
from spotbot.research.rd16j_evaluation import (
...
def test_v2_capacity_configuration_is_five_positions() -> None:
...
def test_v1_comparison_rows_compute_delta() -> None:
    v1 = {
        "net_return": 0.5,
        "cagr": 0.07,
        ...
        "win_rate": 0.4,
        "total_fees": 1000.0,
        "turnover_on_initial_equity": 10.0,
        "minimum_cash": 20000.0,
        "minimum_equity": 95000.0,
    }
    v2 = {
        "net_return": 0.9,
        "cagr": 0.11,
        ...
        "win_rate": 0.41,
        "total_fees": 1500.0,
        "turnover_on_initial_equity": 15.0,
        "minimum_cash": 3000.0,
        "minimum_equity": 94000.0,
    }
    rows = v1_comparison_rows(v1, v2)
    net = next(row for row in rows if row["metric"] == "net_return")
    assert net["absolute_delta"] == 0.4


def test_classify_v2_flags_two_x_cash_constraint() -> None:
    metrics = {
    ...
    concentration = {"top_3_trade_profit_share": 0.12}
    engines = [
        {"positive_net_contribution": True, "net_pnl": 60.0},
        {"positive_net_contribution": True, "net_pnl": 40.0},
    ]
    bull = [
        {
    ...
    assert result.classification == "ROBUST_POSITIVE_V2_WITH_COST_CAPACITY_CONSTRAINT"
