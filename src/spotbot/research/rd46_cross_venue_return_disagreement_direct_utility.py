...
FEATURES = [
    "BTC_ETH_MEAN_SIGNED_CROSS_VENUE_LOG_RETURN_GAP",
    "BTC_ETH_CROSS_VENUE_LOG_RETURN_GAP_DISPERSION",
]
...
def stable_prediction_halves(
    evaluation: pd.DataFrame, predicted: np.ndarray
) -> tuple[pd.DataFrame, pd.DataFrame]:
    work = evaluation.copy()
    work["_predicted_rcv"] = np.asarray(predicted, dtype=float)
    work = work.sort_values(["_predicted_rcv", "decision_id"], kind="mergesort").reset_index(
        drop=True
    )
    n_bottom = len(work) // 2
...
def continuous_metrics(evaluation: pd.DataFrame, predicted: np.ndarray) -> dict[str, Any]:
...
def classification_metrics(evaluation: pd.DataFrame, predicted: np.ndarray) -> dict[str, Any]:
    actual = evaluation["rcv_return"].to_numpy(float)
    event = actual < 0.0
    n_neg = int(event.sum())
...
def subset_metrics(evaluation: pd.DataFrame, predicted: np.ndarray) -> dict[str, Any]:
    work = evaluation.copy()
    work["_predicted_rcv"] = np.asarray(predicted, dtype=float)
    ...
    if len(subset):
        signal_days = int(
            pd.to_datetime(subset["signal_time"], utc=True, errors="raise").dt.date.nunique()
        )
...
def _full_gate_flags(
    train: pd.DataFrame, evaluation: pd.DataFrame, predicted: np.ndarray
) -> dict[str, Any]:
...
def qualify_landmarks(cell_results: list[dict[str, Any]]) -> pd.DataFrame:
...
def persistent_pairs(landmark_df: pd.DataFrame, direction: str) -> list[list[int]]:
    qualified = set(
        landmark_df.loc[
...
def decision_from_persistence(
    ...
    if matching:
        return (
            "RD46_CROSS_VENUE_RETURN_DISAGREEMENT_DIRECT_UTILITY_BIDIRECTIONALLY_PERSISTENT",
            "RD46_P5_PREREGISTER_ISOLATED_ACTION_MAPPING_WITH_SLOT_ESCROW_AND_NO_EARLY_CAPITAL_REUSE",
            matching,
        )
    if forward:
        return (
            "RD46_CROSS_VENUE_RETURN_DISAGREEMENT_DIRECT_UTILITY_FORWARD_ONLY_REGIME_DEPENDENT_NO_CONTEXT_RESCUE",
            "RD46_CLOSE_DIRECT_ACTION_MAPPING_FOR_THIS_SOURCE_AND_RETAIN_FORWARD_ONLY_EVIDENCE_AS_DIAGNOSTIC",
            [],
        )
    return (
        "RD46_CROSS_VENUE_RETURN_DISAGREEMENT_DIRECT_UTILITY_UNQUALIFIED_NO_RESCUE",
        "RD46_CLOSE_OR_PREREGISTER_DISTINCT_DIRECT_UTILITY_INFORMATION_SOURCE_PRE_TARGET_EXPOSURE",
        [],
    )


def strict_join(state: pd.DataFrame, target: pd.DataFrame) -> pd.DataFrame:
    required_state = {
        "decision_id",
        "control_position_id",
        "universe_id",
    ...
    target_keep = ["decision_id", *parity, "target_evaluable", "rcv_return", "right_censored"]
    joined = state.merge(
        target[target_keep],
        on="decision_id",
        how="left",
...
def evaluate(joined: pd.DataFrame) -> dict[str, Any]:
    ...
    for direction, (training_period, evaluation_period) in DIRECTIONS.items():
        for universe in UNIVERSES:
            for landmark in LANDMARKS:
                ...
                cells.append(evaluate_cell(train, ev, universe, landmark, direction))

    landmarks = qualify_landmarks(cells)
    forward = persistent_pairs(landmarks, "FORWARD")
    reverse = persistent_pairs(landmarks, "REVERSE")
    decision, next_stage, matching = decision_from_persistence(forward, reverse)
    return {
        "cells": cells,
        "landmarks": landmarks,
...
def write_outputs(joined: pd.DataFrame, result: dict[str, Any], output_dir: Path) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=False)

    joined.to_csv(output_dir / "joined-cross-venue-return-disagreement-rcv-ledger.csv", index=False)
    ...
    for cell in result["cells"]:
        ...
        if cell["model_fit_pass"]:
            ...
            for name, value in zip(names, cell["coefficients"], strict=True):
                coeff_rows.append({**base, "coefficient_name": name, "coefficient_value": value})
            full = cell["full"]
            cont_rows.append(
                {
                    **base,
    ...
    pd.DataFrame(coeff_rows).to_csv(
        output_dir / "cross-venue-return-disagreement-model-coefficients.csv", index=False
    )
    pd.DataFrame(cont_rows).to_csv(output_dir / "continuous-rcv-cell-evaluation.csv", index=False)
    pd.DataFrame(class_rows).to_csv(
        output_dir / "negative-rcv-classification-cell-evaluation.csv", index=False
    )
    pd.DataFrame(subset_rows).to_csv(
        output_dir / "negative-utility-subset-cell-evaluation.csv", index=False
    )
    pd.DataFrame(lopo_rows).to_csv(output_dir / "leave-one-pair-out-evaluation.csv", index=False)
    result["landmarks"].to_csv(output_dir / "landmark-qualification.csv", index=False)

    temporal = {
        "schema_version": "rd46-p4-temporal-persistence-qualification-v1",
    ...
    forward_cells = [r for r in result["cells"] if r["direction"] == "FORWARD"]
    reverse_cells = [r for r in result["cells"] if r["direction"] == "REVERSE"]
    evidence = {
        "schema_version": "rd46-p4-qualified-direct-utility-evidence-freeze-v1",
        "status": "PASS",
        "decision": result["decision"],
        "next_stage": result["next_stage"],
        "forward_continuous_gate_pass_cell_count": sum(
            r["continuous_gate_pass"] for r in forward_cells
        ),
        "forward_classification_gate_pass_cell_count": sum(
            r["classification_gate_pass"] for r in forward_cells
        ),
        "forward_negative_subset_gate_pass_cell_count": sum(
            r["negative_subset_gate_pass"] for r in forward_cells
        ),
        "forward_joint_qualified_cell_count": sum(r["joint_gate_pass"] for r in forward_cells),
        "reverse_continuous_gate_pass_cell_count": sum(
            r["continuous_gate_pass"] for r in reverse_cells
        ),
        "reverse_classification_gate_pass_cell_count": sum(
            r["classification_gate_pass"] for r in reverse_cells
        ),
        "reverse_negative_subset_gate_pass_cell_count": sum(
            r["negative_subset_gate_pass"] for r in reverse_cells
        ),
        "reverse_joint_qualified_cell_count": sum(r["joint_gate_pass"] for r in reverse_cells),
        "forward_persistent_pairs": result["forward_persistent_pairs"],
        "reverse_persistent_pairs": result["reverse_persistent_pairs"],
        "matching_forward_reverse_persistent_pairs": result["matching_persistent_pairs"],
    ...
    report = {
        **evidence,
        "schema_version": "rd46-p4-direct-utility-report-v1",
    ...
    (output_dir / "rd46-p4-direct-utility-report-v1.json").write_text(
