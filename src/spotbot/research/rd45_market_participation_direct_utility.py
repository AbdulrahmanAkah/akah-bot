...
FEATURES = [
    "PAIR_VS_PIT_PEER_MEDIAN_LOG_24H_QUOTE_TURNOVER",
    "PAIR_VS_PIT_PEER_MEDIAN_12H_LOG_TURNOVER_CHANGE",
]
...
def decision_from_persistence(
    ...
    if matching:
        return (
            "RD45_MARKET_PARTICIPATION_DIRECT_UTILITY_BIDIRECTIONALLY_PERSISTENT",
            "RD45_P5_PREREGISTER_ISOLATED_ACTION_MAPPING_WITH_SLOT_ESCROW_AND_NO_EARLY_CAPITAL_REUSE",
            matching,
        )
    if forward:
        return (
            "RD45_MARKET_PARTICIPATION_DIRECT_UTILITY_FORWARD_ONLY_REGIME_DEPENDENT_NO_CONTEXT_RESCUE",
            "RD45_CLOSE_DIRECT_ACTION_MAPPING_FOR_THIS_SOURCE_AND_RETAIN_FORWARD_ONLY_EVIDENCE_AS_DIAGNOSTIC",
            [],
        )
    return (
        "RD45_MARKET_PARTICIPATION_DIRECT_UTILITY_UNQUALIFIED_NO_RESCUE",
        "RD45_CLOSE_OR_PREREGISTER_DISTINCT_DIRECT_UTILITY_INFORMATION_SOURCE_PRE_TARGET_EXPOSURE",
        [],
    )
...
def write_outputs(joined: pd.DataFrame, result: dict[str, Any], output_dir: Path) -> dict[str, Any]:
    ...
    joined.to_csv(output_dir / "joined-market-participation-rcv-ledger.csv", index=False)
    ...
    pd.DataFrame(coeff_rows).to_csv(
        output_dir / "market-participation-model-coefficients.csv", index=False
    )
    ...
    temporal = {
        "schema_version": "rd45-p4-temporal-persistence-qualification-v1",
    ...
    evidence = {
        "schema_version": "rd45-p4-qualified-direct-utility-evidence-freeze-v1",
        "status": "PASS",
    ...
    report = {
        **evidence,
        "schema_version": "rd45-p4-direct-utility-report-v1",
    ...
    (output_dir / "rd45-p4-direct-utility-report-v1.json").write_text(
