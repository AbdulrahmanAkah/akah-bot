"""RD22 structural risk-on regime-gated pullback discovery engine.

This stage freezes the RD21 best discovery horizon at 168 hours and adds exactly
one causal market-regime gate: the most recent fully completed BTC UTC daily close
must be above its 200-day EMA. Signal, score, entry, static stop, sizing, capacity,
costs, universe, and candidate priority remain unchanged.
"""
...
SCHEMA_VERSION: Final = "rd22-structural-risk-on-pullback-engine-v1"
CANDIDATE_FAMILY_ID: Final = "RD22_STRUCTURAL_RISK_ON_H168_PULLBACK_V1"
DISCOVERY_START: Final = pd.Timestamp("2019-01-01T00:00:00Z")
DISCOVERY_CUTOFF: Final = pd.Timestamp("2022-01-01T00:00:00Z")
HORIZON_HOURS: Final = (168,)
...
class RD22Error(RuntimeError):
    """Raised when the frozen RD21 discovery contract is violated."""


def validate_constants() -> None:
    if HORIZON_HOURS != (168,):
        raise RD22Error("horizon matrix drifted")
    if not math.isclose(BASE_ROUND_TRIP_COST, 0.0025):
        raise RD22Error("base cost drifted")
    if not math.isclose(STRESS_2X_ROUND_TRIP_COST, 0.005):
        raise RD22Error("2x cost drifted")
    if not math.isclose(FIXED_RISK_FRACTION, 0.005):
        raise RD22Error("fixed risk drifted")
    if MAXIMUM_SIMULTANEOUS_POSITIONS != 5:
        raise RD22Error("maximum position count drifted")
    if not math.isclose(MAXIMUM_GROSS_EXPOSURE, 0.9):
        raise RD22Error("gross exposure cap drifted")
    if not math.isclose(MAXIMUM_SINGLE_ASSET_NOTIONAL, 0.4):
        raise RD22Error("single asset cap drifted")
    if not math.isclose(LIQUIDITY_CAPACITY_FRACTION_24H, 0.005):
        raise RD22Error("liquidity capacity cap drifted")


def build_btc_daily_ema200_regime(hourly: pd.DataFrame) -> pd.DataFrame:
    required = {"timestamp", "close"}
    missing = sorted(required.difference(hourly.columns))
    if missing:
        raise RD22Error(f"BTC regime source missing columns: {missing}")

    frame = hourly.loc[:, ["timestamp", "close"]].copy()
    frame["timestamp"] = pd.to_datetime(frame["timestamp"], utc=True, errors="raise")
    frame["close"] = pd.to_numeric(frame["close"], errors="raise")
    frame = frame.loc[frame["timestamp"] < DISCOVERY_CUTOFF,].sort_values(
        "timestamp", kind="stable"
    )
    if frame.empty:
        raise RD22Error("BTC regime source is empty")

    daily = (
        frame.set_index("timestamp")
        .resample("1D", label="left", closed="left")
        .agg(close=("close", "last"), hourly_bar_count=("close", "size"))
        .reset_index()
    )
    daily = daily.loc[daily["hourly_bar_count"] == 24].copy()
    daily["ema200"] = (
        daily["close"]
        .ewm(
            span=200,
    ...
    daily["available_time"] = daily["timestamp"] + pd.Timedelta(days=1)
    daily = daily.loc[daily["available_time"] < DISCOVERY_CUTOFF,].copy()
    daily["regime_ready"] = daily["ema200"].notna()
    daily["regime_on"] = daily["regime_ready"] & (daily["close"] > daily["ema200"])
    return daily.loc[
        :,
        [
            "timestamp",
...
def regime_state_at(
    ...
    if regime.empty:
        return None
    available = pd.DatetimeIndex(regime["available_time"])
    position = int(available.searchsorted(signal_time, side="right")) - 1
    if position < 0:
        return None
    row = regime.iloc[position]
    if not bool(row["regime_ready"]):
        return None
    return bool(row["regime_on"])
...
def replay_fixed_horizon(
    *,
    ...
    if horizon_hours not in HORIZON_HOURS:
        raise RD22Error(f"unauthorized horizon: {horizon_hours}")
    if cost_multiplier not in COST_MULTIPLIERS:
        raise RD22Error(f"unauthorized cost multiplier: {cost_multiplier}")
    ...
    if cost_multiplier == 2.0 and not math.isclose(round_trip, STRESS_2X_ROUND_TRIP_COST):
        raise RD22Error("2x cost calculation drifted")
    side_cost = round_trip / 2.0
    run_id = f"{universe_id}_RISKON_H{horizon_hours}_{cost_multiplier:.0f}x"
    ...
    if selected.empty:
        raise RD22Error(f"no discovery signals for universe {universe_id}")
    ...
    route: dict[str, Any] = {
        ...
        "cost_multiplier": cost_multiplier,
        "signal_candidates": 0,
        "regime_ready_on_candidates": 0,
        "regime_unavailable_rejections": 0,
        "regime_off_rejections": 0,
        "admitted_entries": 0,
    ...
    for timestamp in pd.date_range(start, final_hour, freq="h"):
        ...
        if candidates is not None:
            for raw in candidates.to_dict(orient="records"):
                ...
                regime_state = regime_state_at(regime, signal_time)
                if regime_state is None:
                    route["regime_unavailable_rejections"] += 1
                    continue
                if not regime_state:
                    route["regime_off_rejections"] += 1
                    continue
                route["regime_ready_on_candidates"] += 1
                ...
                if signal_bar is None:
                    raise RD22Error("frozen signal bar vanished")
                ...
                if equity_open <= 0.0:
                    raise RD22Error("portfolio equity became non-positive")
                ...
                if cash < -1e-7:
                    raise RD22Error("negative cash after admission")
                cash = max(cash, 0.0)
    ...
    if trade_frame.empty:
        raise RD22Error(f"discovery replay produced zero trades: {run_id}")
    route.update(
        {
            "min_cash": min_cash,
            "final_cash": cash,
