...
IMPLEMENTATION_SUBJECT: Final = "research(rd16i): register composite alpha v2 architecture"
RESULT_SUBJECT: Final = "research(rd16i): complete composite alpha v2 registration"
GENERATED_DATA: Final = Path("data/research/rd16i")
GENERATED_REPORTS: Final = (
    Path("reports/research/rd16i-composite-alpha-v2-registration-results-v1.md"),
    Path("reports/research/rd16i-causality-capacity-audit-v1.md"),
)
...
def _load_report(repo: Path) -> dict[str, object]:
    path = repo / GENERATED_DATA / "rd16i-final-report-v1.json"
    if not path.is_file():
        raise CompletionError(f"Missing RD16-I final report: {path}")
    raw: object = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(raw, dict):
        raise CompletionError("RD16-I final report must be an object.")
    ...
    for key, expected_value in expected.items():
        if report.get(key) != expected_value:
            raise CompletionError(f"Unexpected RD16-I report field {key}: {report.get(key)!r}")
    technical = report.get("technical_gates")
    if not isinstance(technical, dict):
        raise CompletionError("RD16-I technical_gates is missing.")
    for key in (
        "rd16h_ready",
        ...
        if technical.get(key) is not True:
            raise CompletionError(f"RD16-I technical gate failed: {key}")
    for key in (
        "test_2025_accessed",
        ...
        if technical.get(key) is True:
            raise CompletionError(f"RD16-I forbidden flag is true: {key}")
    return report
...
def _generated_paths(repo: Path) -> list[Path]:
    ...
    if not data_root.is_dir():
        raise CompletionError(f"Missing RD16-I directory: {data_root}")
    paths = sorted(path.relative_to(repo) for path in data_root.iterdir() if path.is_file())
    for report in GENERATED_REPORTS:
        if not (repo / report).is_file():
            raise CompletionError(f"Missing RD16-I report: {report}")
        paths.append(report)
    return sorted(paths)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run, validate, commit and push RD16-I.")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if local != remote:
        raise CompletionError("Local and remote heads differ before RD16-I.")
    subject = _git(repo, "log", "-1", "--pretty=%s")
    if subject != IMPLEMENTATION_SUBJECT:
        raise CompletionError("RD16-I implementation commit is not at HEAD: " + subject)
    tracked = _git(repo, "status", "--short", "--untracked-files=no")
    if tracked:
        raise CompletionError("Tracked changes exist before RD16-I completion:\n" + tracked)

    python = _python(repo)
    targets = (
        "src/spotbot/research/rd16i_architecture.py",
        "src/spotbot/research/rd16i_registration.py",
        "scripts/research/run_rd16i_composite_alpha_v2_registration.py",
        "scripts/research/complete_rd16i_composite_alpha_v2_registration.py",
        "tests/research/test_rd16i_composite_alpha_v2_registration.py",
    )
    ...
    if not data_paths:
        raise CompletionError("No RD16-I data outputs were discovered.")
    ...
    unexpected = [
        ...
        for path in staged
        if not (
            path.startswith("data/research/rd16i/") or path.startswith("reports/research/rd16i-")
        )
    ]
    if unexpected:
        raise CompletionError("Unexpected files staged with RD16-I: " + ", ".join(unexpected))
    ...
    if not arguments.no_push:
        ...
        if result_commit != remote_after:
            raise CompletionError("RD16-I result commit did not reach remote.")
    ...
    if tracked_after:
        raise CompletionError("Tracked changes remain after RD16-I:\n" + tracked_after)
