...
IMPLEMENTATION_SUBJECT = "research(rd16f): register composite alpha architecture"
RESULT_SUBJECT = "research(rd16f): complete composite architecture smoke tests"
GENERATED_DATA = Path("data/research/rd16f")
GENERATED_REPORTS = (
    Path("reports/research/rd16f-composite-architecture-results-v1.md"),
    Path("reports/research/rd16f-composite-routing-causality-audit-v1.md"),
    Path("reports/research/rd16f-component-provenance-constraint-audit-v1.md"),
)
...
def _load_report(repo: Path) -> dict[str, object]:
    path = repo / GENERATED_DATA / "rd16f-final-report-v1.json"
    if not path.is_file():
        raise CompletionError(f"Missing RD16-F final report: {path}")
    raw: object = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(raw, dict):
        raise CompletionError("RD16-F final report must be an object.")
    ...
    for key, value in expected.items():
        if report.get(key) != value:
            raise CompletionError(f"Unexpected RD16-F report field {key}: {report.get(key)!r}")
    technical = report.get("technical_gates")
    if not isinstance(technical, dict):
        raise CompletionError("RD16-F technical_gates is missing.")
    for key in (
        "two_registered_engines",
        "source_components_verified_retained",
        "both_registered_engines_have_candidates",
        "both_registered_engines_have_admitted_trades",
        ...
        "simultaneous_conflicts_resolved_deterministically",
        "duplicate_composite_trade_ids_absent",
        "sealed_cutoff_respected",
        "spot_only",
        "long_only",
    ):
        if technical.get(key) is not True:
            raise CompletionError(f"RD16-F technical gate failed: {key}")
    for key in (
        "test_2025_accessed",
        ...
        if technical.get(key) is True:
            raise CompletionError(f"RD16-F forbidden flag is true: {key}")
    return report
...
def _generated_paths(repo: Path) -> list[Path]:
    ...
    if not data_root.is_dir():
        raise CompletionError(f"Missing RD16-F directory: {data_root}")
    paths = sorted(path.relative_to(repo) for path in data_root.iterdir() if path.is_file())
    for report in GENERATED_REPORTS:
        if not (repo / report).is_file():
            raise CompletionError(f"Missing RD16-F report: {report}")
        paths.append(report)
    return sorted(paths)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run, validate, commit and push RD16-F results.")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if local != remote:
        raise CompletionError("Local and remote heads differ before RD16-F.")
    subject = _git(repo, "log", "-1", "--pretty=%s")
    if subject != IMPLEMENTATION_SUBJECT:
        raise CompletionError("RD16-F implementation commit is not at HEAD: " + subject)
    tracked = _git(repo, "status", "--short", "--untracked-files=no")
    if tracked:
        raise CompletionError("Tracked changes exist before RD16-F completion:\n" + tracked)

    python = _python(repo)
    targets = (
        "src/spotbot/research/rd16f_architecture.py",
        "src/spotbot/research/rd16f_registration.py",
        "scripts/research/run_rd16f_composite_architecture.py",
        "scripts/research/complete_rd16f_composite_architecture.py",
        "tests/research/test_rd16f_composite_architecture.py",
    )
    ...
    if unexpected:
        raise CompletionError("Unexpected RD16-F staged paths:\n" + "\n".join(unexpected))
    required_staged = {
        "data/research/rd16f/architecture-registration.csv",
        "data/research/rd16f/routing-decision-summary.csv",
        "data/research/rd16f/component-provenance.csv",
        "data/research/rd16f/constraint-audit.csv",
        "data/research/rd16f/local-ledger-manifest-v1.json",
        "data/research/rd16f/rd16f-final-report-v1.json",
        "data/research/rd16f/validation-report.json",
        "data/research/rd16f/output-hashes.json",
        "reports/research/rd16f-composite-architecture-results-v1.md",
        "reports/research/rd16f-composite-routing-causality-audit-v1.md",
        "reports/research/rd16f-component-provenance-constraint-audit-v1.md",
    }
    missing_required = sorted(required_staged.difference(staged))
    if missing_required:
        raise CompletionError(
            "Required RD16-F outputs were not staged:\n" + "\n".join(missing_required)
        )
    ...
    if not arguments.no_push:
        ...
        if remote_after != commit:
            raise CompletionError("RD16-F result commit did not reach remote.")
    ...
    print(
        ...
        f"engines_registered={report['engines_registered']}\n"
        f"candidate_count={report['candidate_count']}\n"
        f"admitted_trade_count={report['admitted_trade_count']}\n"
        f"evidence_classification={report['evidence_classification']}\n"
