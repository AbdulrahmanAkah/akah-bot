...
SCHEMA_VERSION = (
    "ams-v1-h03-liquidity-"
    "sweep-reversal-result-v1"
)

EXPECTED_BRANCH = (
    "research/"
    "ams-v1-h03-liquidity-sweep-reversal"
)

EXPECTED_PARAMETER_HASH = (
    "266a93110f083e5b63b849d3524f9cd2"
    "f69e35583ac4f8c8944f51387e328d47"
)
...
OUTPUT_DIRECTORY = (
    ...
    "ams_v1_h03_liquidity_sweep_reversal"
)
...
SIGNALS_PATH = (
    ...
    / (
        "ams-v1-h03-liquidity-"
        "sweep-reversal-signals.parquet"
    )
)
...
WEIGHTS_PATH = (
    ...
    / (
        "ams-v1-h03-liquidity-"
        "sweep-reversal-weights.parquet"
    )
)
...
DAILY_PATH = (
    ...
    / (
        "ams-v1-h03-liquidity-"
        "sweep-reversal-daily-results.parquet"
    )
)
...
def load_history() -> pd.DataFrame:
    ...
    if (
        ...
        raise RuntimeError(
            "Locked 2025+ open-time history entered H03."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Locked post-2024 close-time history entered H03."
        )
...
def load_ranking() -> pd.DataFrame:
    ...
    if (
        ...
        raise RuntimeError(
            "Locked 2025+ ranking entered H03."
        )
...
def registration_from_ledger(
    ...
    if len(matches) != 1:
        raise RuntimeError(
            "Expected exactly one H03 registration."
        )
...
def update_ledgers(
    ...
    if (
        ...
        raise RuntimeError(
            "H03 registration is not REGISTERED."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Frozen H03 parameters do not match."
        )
    ...
    if any(
        ...
        raise RuntimeError(
            "A binding H03 result already exists."
        )
    ...
    if any(
        ...
        raise RuntimeError(
            "Project ledger already contains H03."
        )
...
def main() -> None:
    ...
    if status:
        raise RuntimeError(
            "Working tree must be clean before H03."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Frozen H03 parameter hash mismatch: "
            f"{observed_hash}"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "H03 registration parameters changed."
        )
    ...
    if any(
        ...
        raise RuntimeError(
            "H03 already has a binding result."
        )
    ...
    print(
        "[2/7] Building causal H03 signals..."
    )

    signals = build_liquidity_sweep_signals(
        history,
        ranking,
        policy=h03_policy(
            transaction_cost_fraction=(
    ...
    print(
        "[5/7] Writing H03 research artifacts..."
    )
    ...
    print(
        "[7/7] H03 binding result complete."
    )
