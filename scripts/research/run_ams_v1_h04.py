...
SCHEMA_VERSION = (
    "ams-v1-h04-cross-sectional-"
    "rotation-result-v1"
)

EXPECTED_BRANCH = (
    "research/"
    "ams-v1-h04-cross-sectional-rotation"
)

EXPECTED_PARAMETER_HASH = (
    "9f19f6238fd4c3b19e247fd786dd8026"
    "9b939a0f87f2f8005e54935e831fa3d0"
)
...
OUTPUT_DIRECTORY = (
    ...
    "ams_v1_h04_cross_sectional_rotation"
)
...
SIGNALS_PATH = (
    OUTPUT_DIRECTORY
    / "ams-v1-h04-signals.parquet"
)

WEIGHTS_PATH = (
    OUTPUT_DIRECTORY
    / "ams-v1-h04-weights.parquet"
)

DAILY_PATH = (
    OUTPUT_DIRECTORY
    / "ams-v1-h04-daily-results.parquet"
)

TRADES_PARQUET_PATH = (
    OUTPUT_DIRECTORY
    / "ams-v1-h04-trades.parquet"
)
...
def load_history() -> pd.DataFrame:
    ...
    if not paths:
        raise FileNotFoundError(
            "No research history Parquets were found."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Locked 2025+ open-time history entered H04."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Locked post-2024 close-time history entered H04."
        )
...
def load_ranking() -> pd.DataFrame:
    ...
    if (
        ...
        raise RuntimeError(
            "Locked 2025+ ranking entered H04."
        )
...
def technical_validation(
    ...
    return {
        ...
        "three_day_rebalance_respected": bool(
            rebalance_gaps.empty
            or (
                rebalance_gaps
                == pd.Timedelta(days=3)
            ).all()
...
def registration_from_ledger(
    ...
    if len(matches) != 1:
        raise RuntimeError(
            "Expected exactly one H04 registration."
        )
...
def update_ledgers(
    ...
    if (
        ...
        raise RuntimeError(
            "H04 registration is not REGISTERED."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Frozen H04 parameters do not match."
        )
    ...
    if any(
        ...
        raise RuntimeError(
            "A binding H04 result already exists."
        )
    ...
    if any(
        ...
        raise RuntimeError(
            "Project ledger already contains H04."
        )
...
def main() -> None:
    ...
    if status:
        raise RuntimeError(
            "Working tree must be clean before H04."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Frozen H04 parameter hash mismatch: "
            f"{observed_hash}"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "H04 registration parameters changed."
        )
    ...
    if any(
        ...
        raise RuntimeError(
            "H04 already has a binding result."
        )
    ...
    print(
        "[2/7] Building causal H04 signals..."
    )
    ...
    result = {
        ...
        "limitations": [
            ...
            (
                "Daily data cannot model intraday execution "
                "or order-book impact."
            ),
        ],
    }

    print(
        "[5/7] Writing H04 research artifacts..."
    )
    ...
    print(
        "[7/7] H04 binding result complete."
    )
