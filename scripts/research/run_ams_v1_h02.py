...
HYPOTHESIS_ID = (
    "AMS-V1-H02-"
    "AGGRESSIVE-REACCELERATION"
)

SCHEMA_VERSION = (
    "ams-v1-h02-aggressive-"
    "reacceleration-result-v1"
)

EXPECTED_BRANCH = (
    "research/"
    "ams-v1-h02-aggressive-reacceleration"
)

EXPECTED_PARAMETER_HASH = (
    "1954320dd9355a2c9cd724bb76a2847a"
    "2702c0bb3abba0dfb4f8dc802a749f9f"
)
...
OUTPUT_DIRECTORY = (
    ...
    "ams_v1_h02_aggressive_reacceleration"
)
...
SIGNALS_PATH = (
    ...
    / (
        "ams-v1-h02-aggressive-"
        "reacceleration-signals.parquet"
    )
)
...
WEIGHTS_PATH = (
    ...
    / (
        "ams-v1-h02-aggressive-"
        "reacceleration-weights.parquet"
    )
)
...
DAILY_PATH = (
    ...
    / (
        "ams-v1-h02-aggressive-"
        "reacceleration-daily-results.parquet"
    )
)
...
def load_history() -> pd.DataFrame:
    ...
    if (
        ...
        raise RuntimeError(
            "Locked 2025+ open-time history entered H02."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Locked post-2024 close-time history entered H02."
        )
...
def load_ranking() -> pd.DataFrame:
    ...
    if (
        ...
        raise RuntimeError(
            "Locked 2025+ ranking entered H02."
        )
...
def registration_from_ledger(
    ...
    if len(matches) != 1:
        raise RuntimeError(
            "Expected exactly one H02 registration."
        )
...
def update_ledgers(
    ...
    if (
        ...
        raise RuntimeError(
            "H02 registration is not REGISTERED."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Frozen H02 parameters do not match."
        )
    ...
    if any(
        ...
        raise RuntimeError(
            "A binding H02 result already exists."
        )
    ...
    if any(
        ...
        raise RuntimeError(
            "Project ledger already contains H02."
        )
...
def main() -> None:
    ...
    if status:
        raise RuntimeError(
            "Working tree must be clean before H02."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Frozen H02 parameter hash mismatch: "
            f"{observed_hash}"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "H02 registration parameters changed."
        )
    ...
    if any(
        ...
        raise RuntimeError(
            "H02 already has a binding result."
        )
    ...
    print(
        "[2/7] Building causal H02 signals..."
    )

    primary_policy = h02_policy(
        transaction_cost_fraction=(
            FROZEN_PARAMETERS[
                "transaction_cost_fraction"
            ]
    ...
    print(
        "[5/7] Writing H02 research artifacts..."
    )
    ...
    print(
        "[7/7] H02 binding result complete."
    )
